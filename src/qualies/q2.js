window.apples.qualify2 = window.apples.qualify2 || (function($) {
	'use strict';
	...
	var Qualify = function() {
		...
		function getTemperatureChange() {
			var result, 
				...
				weather = getWeather(),
				rainCoef = [preferences.WetSetup.C.fWing, preferences.WetSetup.C.rWing, preferences.WetSetup.C.engine, preferences.WetSetup.C.brakes, preferences.WetSetup.C.gear, preferences.WetSetup.C.suspension],
				rainCoef2 = [preferences.WetSetup.C2.fWing, preferences.WetSetup.C2.rWing, preferences.WetSetup.C2.engine, preferences.WetSetup.C2.brakes, preferences.WetSetup.C2.gear, preferences.WetSetup.C2.suspension],
				v1 = [preferences.WetSetup.V1.fWing, preferences.WetSetup.V1.rWing, preferences.WetSetup.V1.engine, preferences.WetSetup.V1.brakes, preferences.WetSetup.V1.gear, preferences.WetSetup.V1.suspension],
				v2 = [preferences.WetSetup.V2.fWing, preferences.WetSetup.V2.rWing, preferences.WetSetup.V2.engine, preferences.WetSetup.V2.brakes, preferences.WetSetup.V2.gear, preferences.WetSetup.V2.suspension],
				v3 = [preferences.WetSetup.V3.fWing, preferences.WetSetup.V3.rWing, preferences.WetSetup.V3.engine, preferences.WetSetup.V3.brakes, preferences.WetSetup.V3.gear, preferences.WetSetup.V3.suspension],
			...
			if (preferences.SettingsSetup.TemperatureChange) {
				if (weather[1] !== weather[3]) {
					temperatureDifference = weather[3] - weather[1];

					for (i = 0, iLen = tempCoef.length; i < iLen; i++) {
						title[i] = title[i + rainCoef.length] = title[i + (rainCoef.length * 2)] = countTemperateChangeTitle(tempCoef[i], temperatureDifference);
						q1Numbers[i] = countTemperateChangeHref(tempCoef[i], temperatureDifference, q1Numbers[i]);
					}
				}
			}
			...
			if (weather[0] === 0 && weather[2] === 1) {
				for (i = 0, iLen = rainCoef.length; i < iLen; i++) {
					title[i] = title[i] + countDryWetChangeTitle(rainCoef[i], v1[i], weather[3]);
					href[i] = countDryWetChangeHref(rainCoef[i], v1[i], weather[3], q1Numbers[i]);
					title[i + rainCoef.length] = title[i + rainCoef.length] + countDryWetChangeTitle(rainCoef[i], v2[i], weather[3]);
					href[i + rainCoef.length] = countDryWetChangeHref(rainCoef[i], v2[i], weather[3], q1Numbers[i]);
					title[i + (rainCoef.length * 2)] = title[i + (rainCoef.length * 2)] + countDryWetChangeTitle(rainCoef2[i], v3[i], weather[3]);
					href[i + (rainCoef.length * 2)] = countDryWetChangeHref(rainCoef2[i], v3[i], weather[3], q1Numbers[i]);
				}
			} else if (weather[0] === 1 && weather[2] === 0) {
				for (i = 0, iLen = rainCoef.length; i < iLen; i++) {
					title[i] = title[i] + countWetDryChangeTitle(rainCoef[i], v1[i], weather[3]);
					href[i] = countWetDryChangeHref(rainCoef[i], v1[i], weather[3], q1Numbers[i]);
					title[i + rainCoef.length] = title[i + rainCoef.length] + countWetDryChangeTitle(rainCoef[i], v2[i], weather[3]);
					href[i + rainCoef.length] = countWetDryChangeHref(rainCoef[i], v2[i], weather[3], q1Numbers[i]);
					title[i + (rainCoef.length * 2)] = title[i + (rainCoef.length * 2)] + countWetDryChangeTitle(rainCoef2[i], v3[i], weather[3]);
					href[i + (rainCoef.length * 2)] = countWetDryChangeHref(rainCoef2[i], v3[i], weather[3], q1Numbers[i]);
				}
			}
			...
			if (!preferences.SettingsSetup.TemperatureChange) {
				if (weather[1] !== weather[3]) {
					...
					for (i = 0, iLen = href.length; i < iLen; i++) {
						title[i] = title[i] + countTemperateChangeTitle(tempCoef[i % 6], temperatureDifference);
						href[i] = countTemperateChangeHref(tempCoef[i % 6], temperatureDifference, href[i]);
					}
				}
			}
			...
			if (href.length === 6) {
				result = '<a class="lime" href="javascript: QuickLink({href})" title="Change: {title}">Temperature change</a>';

				if (title.length) {
					temp = title[0] + ', ' + title[1] + ', ' + title[2] + ', ' + title[3] + ', ' + title[4] + ', ' + title[5];
					result = result.replace(/{title}/g, temp);
				} else {
					result = result.replace(/{title}/g, '-');
				}

				temp = href[0] + ',' + href[1] + ',' + href[2] + ',' + href[3] + ',' + href[4] + ',' + href[5];

				result = result.replace(/{href}/g, temp);
			} else {
				result = '<a class="lime" href="javascript: QuickLink({href1})" title="Change: {title1}">Version 1</a>&nbsp;<a class="lime" href="javascript: QuickLink({href2})" title="Change: {title2}">Version 2</a>&nbsp;<a class="lime" href="javascript: QuickLink({href3})" title="Change: {title3}">Version 3</a>';
				...
				if (title.length) {
					temp = title[6] + ', ' + title[7] + ', ' + title[8] + ', ' + title[9] + ', ' + title[10] + ', ' + title[11];
					result = result.replace(/{title2}/g, temp);
				} else {
					result = result.replace(/{title2}/g, '-');
				}

				temp = href[6] + ',' + href[7] + ',' + href[8] + ',' + href[9] + ',' + href[10] + ',' + href[11];

				result = result.replace(/{href2}/g, temp);

				if (title.length) {
					temp = title[12] + ', ' + title[13] + ', ' + title[14] + ', ' + title[15] + ', ' + title[16] + ', ' + title[17];
					result = result.replace(/{title3}/g, temp);
				} else {
					result = result.replace(/{title3}/g, '-');
				}

				temp = href[12] + ',' + href[13] + ',' + href[14] + ',' + href[15] + ',' + href[16] + ',' + href[17];
