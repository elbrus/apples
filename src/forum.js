window.apples.forum =
	...
	(function (
				...
				$.ajax({
					...
					success: function (data) {
						if (data) {
							...
							params.ID = $("#managerinformation, .xmanagerinfo", data)
								.find('a[href^="Manager"]')
								.attr("href")
								.split("=")[1];
							...
							if (link.length)
								params.NextRace = link.attr("href").split("=")[1];
			...
			function checkTestingPracticeOccurrence(defer) {
				$.ajax({
					...
					success: function (data) {
						...
						params.TestsBeforePractice =
							messages.indexOf("Testing session costs") <
							messages.indexOf("Qualify 2 lap costs");
						defer.resolve();
					},
					error: function () {
						defer.resolve();
		...
		window.apples.driver,
		window.apples.director,
