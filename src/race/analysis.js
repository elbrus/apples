window.apples.analysis = window.apples.analysis || (function($) {
	'use strict';
	...
	var RaceAnalysis = function() {
		...
		function prepareInfo(data) {
			...
			return {
				...
				Pits: pitsData,
				CarParts: $.map($('table:eq(' + (2 + isPracticeSet) + ') tr:gt(1)', data).filter(function(index) { return index % 2 === 0; }), function(row) {
				...
				CarProblem: $('font[color="orange"]', data).filter(function() {
					return $(this).text().trim() === 'Car problem';
				}).first().parents('tr:first').find('a').text(),
				StartAccident: $('font[color="orange"]', data).filter(function() {
					return $(this).text().trim() === 'Start accident';
				}).length === 1,
				TechnicalProblems: tpData,
				Energy: $('.barLabel', data).map(function() {
					return parseInt($(this).text(), 10);
				}).toArray()
			};
		}
			...
			if (preferences.SettingsSetup.RA.Avg) {
				...
				if (tableElem.parent().attr('id') !== 'dvFinAnalisysTable' && tableElem.text().indexOf('Technical problems') === -1) {
					...
					rows.each(function(index) {
						...
						if (!index) {
							row.append('<th class="center">Avg.<br>temp.</th><th class="center">Avg.<br>hum.</th>');
						} else {
							var pitLap = parseInt(row.find('td:first').text().match(/\d+/g)[1], 10),
								sum = countAverage(data, lap, pitLap);

							row.append('<td class="center"><b>' + sum[0] + '&deg;</b></td><td class="center"><b>' + sum[1] + '%</b></td>');
					...
					if (finishData.length) {
						...
						tableElem.append('<tr><td nowrap="" class="center">Last<br>Stint</td><td class="center">&nbsp;</td><td class="center"><font color="#00ddff"><b>' + tyres + '%</b></font></td><td class="center"><b>' + fuel + 'l</b></td><td class="center">&nbsp;</td><td class="center">&nbsp;</td><td class="center"><b>' + sum[0] + '&deg;</b></td><td class="center"><b>' + sum[1] + '%</b></td></tr>');
					}
				}
			}
		};
