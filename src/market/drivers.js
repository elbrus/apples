		...
		var DriversMarket = function () {
			...
			this.addFunctionality = function () {
				...
				driversTable.find("tr:first th").each(function (column) {
					...
					if (
						headerText === "minimal salary" ||
						headerText === "minimal signing fee" ||
						headerText === "rank"
					) {
						header
							.click(function () {
								...
								rows.sort(function (a, b) {
									return (
										_this.parseCell(a, column).replace(/\$|\./g, "") -
										_this.parseCell(b, column).replace(/\$|\./g, "")
									);
								});
					...
					if (
						...
						header
							.click(function () {
								...
								rows.sort(function (a, b) {
									return (
										_this.parseCell(a, column) - _this.parseCell(b, column)
									);
								});
				...
				driversTable.find("tr").each(function (index) {
					...
					if (!index) {
						...
						row.append(
							'<td id="' +
								$("td a:first", this).attr("href") +
								'" class="fastView" style="cursor: pointer">[+]</td>'
						);
					}
				});
				...
				$(".fastView").on("click", function (event) {
					...
					Driver.getSkills(popup.attr("id"), function (skills) {
						$("body").append(skills);
						$(".driverPopup")
							.css({ top: event.pageY, left: event.pageX - 160 })
							.show();
					});
				});
			};
